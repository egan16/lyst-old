...
<header class="flex items-center justify-between flex-wrap mx-8 my-3">
    <div class="flex items-center flex-shrink-0">
        <h1 class="w-16 text-4xl text-mineshaft">Lysts</h1>
    </div>
...
<div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
    ...
    <article class="overflow-hidden rounded-lg shadow-lg">
        ...
        <header class="flex items-center justify-between leading-tight p-2 md:p-4">
            <h1 class="text-2xl">
                <a class="no-underline hover:text-safetyorange text-mineshaft" href="#">
                    Hardcoded list
                </a>
            </h1>
        ...
        <div class="mt-4 sm:mt-0 sm:ml-4 text-center sm:text-left mb-4">
            <p class="text-sm leading-tight text-gray-600">Public</p>
            <p class="text-sm leading-tight text-gray-600">user1</p>
            <div class="mt-4">
                <button class="text-purple-500 hover:text-white hover:bg-purple-500 border border-purple-500 text-xs font-semibold rounded-full px-4 py-1 leading-normal">Edit</button>

                <form style="display:inline-block" method="POST" action="{{ route('user.lists.destroy', $listModel->id) }}">
                    <input type="hidden" name="_method" value="DELETE">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="submit" class="text-purple-500 hover:text-white hover:bg-purple-500 border border-purple-500 text-xs font-semibold rounded-full px-4 py-1 leading-normal">Delete</button>
